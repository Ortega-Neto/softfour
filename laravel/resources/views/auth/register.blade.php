<!DOCTYPE html>
<html lang="pt-br">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Cadastrar-se</title>
    
    <link rel="stylesheet" type="text/css" href="{{ asset('css/app.css') }}">
    <link rel="stylesheet" type="text/css" href="{{ asset('css/style.css') }}">

</head>

<body class="bg" background="{{asset('storage/imagens/background.jpeg')}}">

    <center>
        <div class="card">
            <slot name="logo">
                <a href="{{route('inicial')}}">
                    <img class="fill-current text-gray-500" src='storage/imagens/logo.png'/>
                </a>
            </slot>

            <!-- Validation Errors -->
            <x-auth-validation-errors class="mb-4" :errors="$errors" />
            
            <form method="POST" action="{{ route('register') }}">
                @csrf

                <!-- Nome -->
                <div class="ml-4 mr-4 mt-4">
                    <x-input id="name" class="block mt-1 w-full" placeholder="Nome" type="text" name="name" required autofocus />
                </div>
 
                <!-- Email Address -->
                <div class="ml-4 mr-4 mt-4">
                    <x-input id="email" class="block mt-1 w-full" placeholder="Email" type="email" name="email" required />
                </div>

                <!-- Password -->
                <div class="ml-4 mr-4 mt-4">
                    <x-input id="password" class="block mt-1 w-full" placeholder="Senha"
                                    type="password"
                                    name="password"
                                    required autocomplete="new-password" />
                </div>

                <!-- Confirm Password -->
                <div class="ml-4 mr-4 mt-4">
                    <x-input id="password_confirmation" class="block mt-1 w-full" placeholder="Confirmação de Senha"
                                    type="password"
                                    name="password_confirmation" required />
                </div>

                <div class="flex items-center justify-center mt-4">
                    <button class="ml-3 button button1"> 
                        {{ __('Realizar Cadastro') }}
                    </button>
                </div>
            </form>
        </div>
    </center>
    <div class="card-rp">
        <div class="tx">
            © 2020 MiCs Minha Criticas. Todos os direitos reservados.
        </div>
    </div>
</body>

</html>